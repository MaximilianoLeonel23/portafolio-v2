...
const Navbar: React.FC<Props> = ({ theme, setTheme, handleShowMenu, changeBtn }) => {
		...
		<nav>
			...
			<ul className='flex md:hidden items-center gap-6'>
				...
				<li>
					...
					>
						<span id='line1' className={`w-6 h-0.5 rounded-lg navbar-menu-${theme} transition`}></span>
						<span
							id='line2'
							className={`w-6 h-0.5 -translate-x-1 rounded-lg navbar-menu-${theme} transition`}
						></span>
						<span id='line3' className={`w-6 h-0.5 rounded-lg navbar-menu-${theme} transition`}></span>
					</div>
				</li>
			</ul>
		</nav>
